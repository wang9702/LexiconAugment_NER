    ...
    for idx in range(len(in_lines)):
        ...
        # 遇到空行，长度为1，一句话结束
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words) > 0):
                ...
                w_length = len(words)
                # 包含word[i]的id列表
                gazs = [[[] for _ in range(4)] for _ in range(w_length)]         # gazs:[c1,c2,...,cn]  ci:[B,M,E,S]  B/M/E/S :[w_id1,w_id2,...]  None:0
                # 包含word[i]的id数量，需要相加
                gazs_count = [[[] for _ in range(4)] for _ in range(w_length)]
                gaz_char_Id = [[[] for _ in range(4)] for _ in range(w_length)]  # gazs:[c1,c2,...,cn]  ci:[B,M,E,S]  B/M/E/S :[[w1c1,w1c2,...],[],...]
                max_gazlist = 0
                max_gazcharlen = 0
